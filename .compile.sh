...
path=$(readlink -f "$1") 	# whole path
dir="$(dirname "$path")/"	# directory only (with trailing /)
...
# generate the .ps output and grep the numbers of the TOC into nb (there is some noise in the first referenced numbers
# -m causes groff to load preprocess file but it breaks page numbering, so it got removed
cat "$marked" \
...
# add the numbers to pretoc into toc
maketoc "$pretoc" $numbers > "$toc"
# insert toc in the original document
cat "$marked" \
